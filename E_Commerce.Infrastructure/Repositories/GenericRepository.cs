...
namespace E_Commerce.Infrastructure.Repositories
{
    internal class GenericRepository<TEntity, TKey>(StoreDbContext dbContext) : IGenericRepository<TEntity, TKey> where TEntity : BaseEntity<TKey>
    {
        ...
        {
            return await SpecificationEvaluator.GreateQuery(dbContext.Set<TEntity>(), spec).CountAsync(ct);
        }
        ...
        {
            var query = SpecificationEvaluator.GreateQuery(dbContext.Set<TEntity>(), Spec);

            return await query.ToListAsync(ct);
        }

        public async Task<TEntity?> GetByIdAsync(TKey id, CancellationToken ct = default) => await dbContext.Set<TEntity>().FindAsync(new object[] { id }, ct);
