...
namespace E_Commerce.Application.Services
{
    internal class BasketService : IBasketService
    {
        ...
        {
            // map Basket Dto to CustomerBasket 

            var CustomerBasket = mapper.Map<CustomerBasket>(basket);
            var BasketResult =await  _basketRepository.CreateOrUpdateBasketAsync(CustomerBasket, TLV, ct);


            return (Result<BasketDto>)(BasketResult == null ? Result<BasketDto>.Fail(Error.Failure("BasketCreate.Failure ", "Can Not Create Or Update Basket "))
                : Result<BasketDto>.Ok());
